...
\section{まえがき及びテーマ}
どうも、Metalloid.です。前回の砕月でライト回にするとか言いながら文章量が爆発しました。反省しています（解決策は見つかっていません）。と云いつつも、文ちゃん20周年を祝して（去年の京都合同が同人イベント初参加だったという個人的な思い入れもありますが）今回はかなり重たくしたいと思ってしまい、風神少女のハーモニーを分析しようと思い至りました。\par
...
\subsubsection{二番}
最後に、気になる方も多いかと思うので、一応一番と二番の間の間奏に少し触れようと思います（ここから少しミクロな議論が続くので、難しいと思う方は本節最後の段落まで飛ばしても構いません）。
\begin{figure}
    \centering
...
\end{itemize}
定義だけをみてもイメージがわかないと思うので、実例を見ていきましょう。また、\autoref{fig:neoRiemannianTransforms}にも変換のイメージを図示しています。
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{neo riemannian transforms.png}
    \caption{R, P, L変換}
    \label{fig:neoRiemannianTransforms}
\end{figure}
\begin{itemize}
...
\begin{figure}
    ...
    \caption{ネオリーマン理論の基本変換によって張られるコード空間（左下がり:R、上下:P、右下がり:L）}
    \label{fig:chordSpace}
\end{figure}
\hdashrule[0.5ex][c]{0.5\linewidth}{0.5pt}{1.5mm}\par
\par
\textbf{以下オタク向け注}\par
ネオリーマン理論の基本変換が定めるコード空間を図にまとめた有名な図形の一つとして、\ruby{トネッツ}{Tonnetz}があります。これは\autoref{fig:tonnetz}のような図形で、上三角形あるいは下三角形がコードを表し、三角形の頂点が構成音を表します。三角形を各方向について折り返することで変換を表現しており、この空間を歩くことによってネオリーマン理論の基本変形を施すことができるといった図形です。この図を見ていると気づく点として、図の上端と下端がパ〇クマンおよびク〇ノトリガーのマップのように結びつくことです（左右に関しても同様に）。つまり、ネオリーマン理論の基本的コード変形が張るコード空間はトーラス空間と同相であることがわかります！
\par
\hdashrule[0.5ex][c]{0.5\linewidth}{0.5pt}{1.5mm}\par
\par
\begin{figure}
