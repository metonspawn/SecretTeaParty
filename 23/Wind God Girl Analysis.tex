...
\vspace{10pt}
\textbf{コードの展開形}\par
上記がコードの基本的な定義となりますが、そこから定義を少し延長して、コードの構成音の存在のみを条件にすると、新たな構成音の配置の仕方が可能になります。例えば、\wc{E}コードを、「EとG\sh とBの音があわさってつくる和音」と定義を広げると、\autoref{fig:inversions}のように、構成音の配置を変更してもコード自体は変わらないことになります（「\wc{E}」から「\wc{G#6omit5}」等にならず、常に「\wc{E}」の家族のまま）。このようにコードの構成音の「順番」を並び替えることによって新たな配置を生むことを「展開」と呼び、それによって生まれるコードの配置を「展開形」と呼びます。また、そのコードシンボルは「元々のコード」/「新たに最低音になった音」と書きます。
\begin{figure}
    \begin{subfigure}{0.4\textwidth}
        \centering
